...
class ShapeNet(object):
    ...
    def get_random_batch(self, batch_size):
        ...
        for i in range(batch_size):
            ...
            # Get a random object from within this class
            object_id = np.random.randint(0, self.num_data[class_id])

            viewpoint_id_a = int(np.random.randint(0, 12) * 2)
            viewpoint_id_b = int(np.random.randint(0, 12) * 2)
    ...
    def get_random_vsl_batch(self, batch_size):
        data_ids_a = np.zeros(batch_size, 'int32')
        data_ids_b = np.zeros(batch_size, 'int32')
        viewpoint_ids_a = torch.zeros(batch_size)
        viewpoint_ids_b = torch.zeros(batch_size)
        ...
        images_a = torch.from_numpy(self.images[data_ids_a].astype('float32') / 255.)
        images_b = torch.from_numpy(self.images[data_ids_b].astype('float32') / 255.)
        voxels_a = torch.from_numpy(self.voxels[data_ids_a // 24].astype('float32'))
        voxels_b = torch.from_numpy(self.voxels[data_ids_a // 24].astype('float32'))
        ...
        return images_a, images_b, viewpoints_a, viewpoints_b, voxels_a, voxels_b

    def get_all_batches_for_evaluation(self, batch_size, class_id):
        data_ids = np.arange(self.num_data[class_id]) + self.pos[class_id]
        viewpoint_ids = np.tile(np.arange(0, 24, 2), data_ids.size)
        data_ids = np.repeat(data_ids, 12) * 24 + viewpoint_ids
