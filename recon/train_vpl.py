...
parser.add_argument('-sample', '--subsample', type=str, default=False)
...
# model = models_vpl.Model('data/obj/sphere/sphere_642.obj', args=args)
print("Made model")
model = model.cuda()
...
dataset_train = None
if args.subsample:
    dataset_train = datasets_subsample.ShapeNet(args.dataset_directory, args.class_ids.split(','), 'train')
else:
    dataset_train = datasets.ShapeNet(args.dataset_directory, args.class_ids.split(','), 'train')
print("Loaded dataset_train")
dataset_val = None
if args.subsample:
    dataset_val = datasets_subsample.ShapeNet(args.dataset_directory, args.class_ids.split(','), 'val')
else:
...
def train():
    ...
    for i in range(start_iter, args.num_iterations + 1):
        ...
        # VAL
        # load images from multi-view
        val_images_a, val_images_b, val_viewpoints_a, val_viewpoints_b = dataset_val.get_random_batch(args.batch_size)
        ...
        # soft render images
        val_render_images, val_laplacian_loss, val_flatten_loss = model([val_images_a, val_images_b], 
