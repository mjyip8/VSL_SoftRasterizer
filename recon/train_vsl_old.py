...
model = models_vsl.Model('data/obj/sphere/sphere_642.obj', args=args)
print("Made model")
model = model.cuda()
...
def train():
    ...
    for i in range(start_iter, args.num_iterations + 1):
        ...
        voxels, images = dataset_train.get_random_vsl_batch(args.batch_size)
        images = images.cuda()
        voxels = voxels.cuda()

        # soft render images
        render_images, laplacian_loss, flatten_loss = model([voxels, images], 
                                                            task='train')
